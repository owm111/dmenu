...
setup(void)
{
	...
	drw_clr_set_alpha(&scheme[SchemeNorm][ColBg], alpha);
...
usage(void)
{
	die("usage: dmenu [-bcfinstv] [-a alpha] [-l lines] [-h height] [-p prompt] [-fn font] [-m monitor]\n"
	    "             [-mw width] [-nb color] [-nf color] [-sb color] [-sf color] [-w windowid]\n");
}
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		/* these options take one argument */
			...
			lines = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-a"))   /* alpha for color[SchemeNorm][ColBg] */
			alpha = atof(argv[++i]);
